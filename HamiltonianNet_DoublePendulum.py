...
from utils_DoublePendulum import symEuler, DoublePendulum_exact, DoublePendulumsolution, energy ,saveData
...
def dfx(x,f):
    return grad([f], [x], grad_outputs=torch.ones(x.shape, dtype=dtype, device=device), create_graph=True)[0]

def perturbPoints(grid,t0,tf,sig=0.5):
#   stochastic perturbation of the evaluation points
#   force t[0]=t0  & force points to be in the t-interval
    delta_t = grid[1] - grid[0]  
    noise = delta_t * torch.randn_like(grid).to(device) * sig
    t = grid + noise
    t.data[2] = torch.ones(1,1, device=device)*(-1)
...
def parametricSolutions(t, nn, X0):
    ...
    N1, N2, N3, N4 = nn(t)#.to(device=device))
    dt =t-t0
#### THERE ARE TWO PARAMETRIC SOLUTIONS. Uncomment f=dt 
    f = (1-torch.exp(-dt))#.to(device=device)
#     f=dt
    x_hat  = x0  + f*N1
    y_hat  = y0  + f*N2
    px_hat = px0 + f*N3
...
def hamEqs_Loss(t,x,y,px,py,lam):
    ...
    fx  = xd - (px-py*torch.cos(x-y))/(1+(torch.sin(x-y)).pow(2));# edit

    fy  = yd - (2*py-px*torch.cos(x-y))/(1+(torch.sin(x-y)).pow(2)); 

    fpx = pxd - (-2*g*torch.sin(x) - ((px*py*torch.sin(x - y)) / (1 + (torch.sin(x- y)).pow(2))) \
+ torch.sin(2*(x - y))*((px.pow(2) + 2*py.pow(2) - 2*px*py*torch.cos(x - y)) / (2 + 2*(torch.sin(x- y)).pow(2)).pow(2)))
    
    fpy = pyd - (-g*torch.sin(y) + ((px*py*torch.sin(x - y)) / (1 + (torch.sin(x- y)).pow(2))) \
- torch.sin(2*(x - y))*((px.pow(2) + 2*py.pow(2) - 2*px*py*torch.cos(x - y)) / (2 + 2*(torch.sin(x- y)).pow(2)).pow(2)))
...
def hamiltonian(x,y,px,py,lam):
    ...
    #V = 0.5*(x**2 + y**2) + lam*(x**2*y - y**3/3) # edit
    #K = 0.5*(px**2+py**2)
    ham = ((px**2 + 2*py**2 - 2*px*py*np.cos(x - y)) / (2 + 2*(np.sin(x - y))**2)) \
                                - 2*g*(np.cos(x) + 0.5*np.cos(y))
    return ham

def hamiltonian_tensor(x,y,px,py,lam):
    #returns the hamiltonian ham for Kinetic (K)  and Potential (V) Energies
    ham = ((px.pow(2) + 2*py.pow(2) - 2*px*py*torch.cos(x - y)) / (2 + 2*(torch.sin(x - y)).pow(2))) \
                                - 2*g*(torch.cos(x) + 0.5*torch.cos(y))
    return ham
...
# Define the loss function as the time derivative of the hamiltonian
    ...
    ham = ((px.pow(2) + 2*py.pow(2) - 2*px*py*torch.cos(x - y)) / (2 + 2*(torch.sin(x - y)).pow(2))) \
                                - 2*g*(torch.cos(x) + 0.5*torch.cos(y))
     # edit
    hx  = grad([ham], [x], grad_outputs=torch.ones(x.shape, dtype=dtype, device=device), create_graph=True)[0]
    hy  = grad([ham], [y], grad_outputs=torch.ones(y.shape, dtype=dtype, device=device), create_graph=True)[0]
    hpx = grad([ham], [px], grad_outputs=torch.ones(px.shape, dtype=dtype, device=device), create_graph=True)[0]
    hpy = grad([ham], [py], grad_outputs=torch.ones(py.shape, dtype=dtype, device=device), create_graph=True)[0]
...
class odeNet_DoublePendulum_MM(torch.nn.Module):
    def __init__(self, D_hid=10):
        super(odeNet_DoublePendulum_MM,self).__init__()

        # Define the Activation
        self.actF = torch.nn.Sigmoid()   # edit different activation function
        #self.actF = mySin()
        
        # define layers
        self.Lin_1   = torch.nn.Linear(1, D_hid)
        self.Lin_2   = torch.nn.Linear(D_hid, D_hid)
        # edit: add another layer
...
# Train the NN
def run_odeNet_DoublePendulum_MM(X0, tf, neurons, epochs, n_train,lr, PATH= "models/model_DoublePendulum", loadWeights=False,
                     minLoss=1e-3):
                    
    fc0 = odeNet_DoublePendulum_MM(neurons).to(device) #edit
    fc1 =  copy.deepcopy(fc0).to(device) # fc1 is a deepcopy of the network with the lowest training loss
    # optimizer
    betas = [0.999, 0.9999]
    
    optimizer = optim.Adam(fc0.parameters(), lr=lr, betas=betas) #, weight_decay=1e-4)
    Loss_history = [];     Llim =  1 
    Loss_erg_history= []
...
#  Network solutions 
        x,y,px,py =parametricSolutions(t,fc0,X0) # t.to(device) edit
...
# LOSS FUNCTION
    ...
    # ENERGY REGULARIZATION
        #print(type(x)) tensor type
        ham  = hamiltonian_tensor(x,y,px,py,lam)
...
# OPTIMIZER
        ...
        optimizer.step() 
        loss = Ltot.item()  #.data.numpy()
        loss_erg=L_erg.item() #.data.numpy()
        optimizer.zero_grad()
...
#Keep the best model (lowest loss) by using a deep copy
        if  tt > 0.8*epochs  and Ltot < Llim:
            fc1 =  copy.deepcopy(fc0).to(device)
            Llim=Ltot 
...
def trainModel(X0, t_max, neurons, epochs, n_train, lr,  loadWeights=True, minLoss=1e-6, showLoss=True, PATH ='models/'):
    model,loss,runTime, loss_erg = run_odeNet_DoublePendulum_MM(X0, t_max, neurons, epochs, n_train,lr,  loadWeights=loadWeights, minLoss=minLoss)
    ...
    if showLoss==True :
        ...
        plt.savefig('DoublePendulum_loss.png')
    

def loadModel(PATH="models/model_DoublePendulum"):
    if path.exists(PATH):
        fc0 = odeNet_DoublePendulum_MM(neurons).to(device) #edit
        checkpoint = torch.load(PATH)
        fc0.load_state_dict(checkpoint['model_state_dict'])
...
t0, x0, y0, px0, py0, lam =  0, np.pi * (20./180.), np.pi * (35./180.), 0, 0, 1; # edit: less chaotic intialit conditions
...
# first do very short time and then load weights into long time training

## LONG TIME: use loadWeights=True
# t_max, N =  3*np.pi, 1800; 
# print(t_max * 0.069, ' Lyapunov times prediction'); dt = t_max/N; 
# n_train, neurons, epochs, lr = N, 80, int(5e4 ), 8e-3
# # TRAIN THE NETWORK. 
...
x,y,px,py =parametricSolutions(tTest,model,X0) # tTest.to(device) edit
...
E0, E_ex = DoublePendulum_exact(N,x0, y0, px0, py0, lam)
x_num, y_num, px_num, py_num = DoublePendulumsolution(N,t_num, x0, y0, px0, py0, lam)
...
plt.plot(t_net, E,'--b', label='Neural Net')
#plt.plot(t_s,E_s,':k',linewidth=lineW); 
plt.plot(t_s10,E_s10,'-.r',linewidth=lineW, label='Symplectic Euler x 1000 points'); 
...
plt.tight_layout()
plt.savefig('DoublePendulum_trajectories.png')
...
#x_numN, y_numN, px_numN, py_numN   = DoublePendulumsolution(Ns,t_s, x0, y0, px0, py0, lam) 
#dx_s = x_numN - x_s;        dpx_s = px_numN - px_s
#dy_s = y_numN - y_s;        dpy_s = py_numN - py_s

x_numN, y_numN, px_numN, py_numN   = DoublePendulumsolution(Ns10,t_s10, x0, y0, px0, py0, lam) 
...
plt.tight_layout()
plt.savefig('DoublePendulum_trajectories_error.png')
